...
conf_mark=`awk '/batchsize|lr|num_frames|subsampling|window_size/ {sub(/: /,"_"); print $1} /optimizer|rnn_cell/ {print $2}' conf/$model_name/train.yaml|
            paste -s -d '_'`

pretrain_model=exp/$pretrained_set/$model_name/$conf_mark/models/avg.th
...
exp_dir=exp/adapt_callhome/${model_name}_${pretrained_set}/$conf_mark
...
if [ $stage -le 1 ]; then
    echo "Start adapting"
    python run/train.py -c $adapt_conf $train_adapt_dir $dev_adapt_dir $model_adapt_dir \
...
if [ $stage -le 3 ]; then
    ...
    python run/infer.py -c $infer_conf $dev_adapt_dir $avg_model $infer_out_dir
fi
...
scripts/best_score.sh $scoring_dir
