...
if [ "$exp_dir" = "" ];then  
    ...
    # exp_dir=exp/${dataset}_v/$model/${conf_mark}${suffix}
fi
...
if [ $dataset = "mini_LibriSpeech" ]; then
    ...
    # train_dir=data/$dataset/data/train_clean_360_nsv_90000
...
if [ $stage -le 1 ]; then
    ...
    python run/train.py -c $train_conf -f $feature_conf $train_dir $dev_dir $model_dir --max-epochs $max_epoch --num-speakers $num_speaker --resume $checkpoint #--chunk-size $chunk_size --num-frames $num_frames
...
if [ $stage -le 3 ]; then
    ...
    python run/infer.py -c $infer_conf -c2 $model_dir/param.yaml -f $feature_conf $dev_dir $avg_model $infer_out_dir --num-speakers $num_speaker #--chunk-size $chunk_size
fi
...
if [ $stage -le 5 ]; then
    ...
    --num_speaker 4 --dev_adapt_dir data/LibriSpeech/data/dev_clean_ns4_beta12_500
    # ./scripts/infer.sh --model $model --pretrain_exp_dir $exp_dir \
