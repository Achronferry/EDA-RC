...
if [ "$exp_dir" = "" ];then  
    ...
    exp_dir=exp/${dataset_name}_${num_speaker}/$model_name/${conf_mark}${suffix}
fi
...
if [ $stage -le 1 ]; then
    ...
    python run/train_v2.py -c $train_conf -f $feature_conf $train_dir $dev_dir $model_dir --max-epochs $max_epoch --resume $checkpoint
...
if [ $stage -le 3 ]; then
    ...
    python run/infer_v2.py -c $infer_conf -c2 $model_dir/param.yaml -f $feature_conf $dev_dir $avg_model $infer_out_dir
fi
