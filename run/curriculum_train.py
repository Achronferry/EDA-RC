...
def train(args):
    ...
    for epoch in range(args.resume + 1, args.max_epochs + 1):
        ...
        for step, (y, t) in tqdm(enumerate(train_iter), ncols=100, total=len(train_iter)):
            ...
            loss_epoch = [loss_epoch[i] + losses[i].item() for i in range(len(losses))] if loss_epoch is not None else losses
...
if __name__=='__main__':
    ...
    parser.add_argument('--chunk-size', default=0, type=int,
                        help='chunk size of long sequence model')
    ...
    #                     help='whether use curriculum learning strategy')
